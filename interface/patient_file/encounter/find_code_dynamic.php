...
<script>
    ...
    function selcode(codetype, code, selector, codedesc, modifier,target_element, limit = 0) {
        if (opener.closed || (!opener.set_related && !opener.set_related_target)) {
            ...
            if (target_element != '') {
                var msg = opener.set_related_target(codetype, code, selector, codedesc, modifier,target_element, limit);
            } else if (singleCodeSelection == '1') {
                opener.promiseData = JSON.stringify({codetype, code, selector, codedesc,modifier});
                dlgclose();
            } else {
