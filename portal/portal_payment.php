...
$patdata = sqlQuery("SELECT " . "p.fname, p.mname, p.lname, p.postal_code, p.pubpid,p.pid, i.copay, i.provider " . "FROM patient_data AS p " . "LEFT OUTER JOIN insurance_data AS i ON " . "i.pid = p.pid AND i.type = 'primary' " . "WHERE p.pid = ? ORDER BY i.date DESC LIMIT 1", array($pid
));
...
//no checks taken here....
    ...
    <form id="invoiceForm" method='post' action='<?php echo $GLOBALS["webroot"] ?>/portal/portal_payment.php'>
        ...
        <table>
            ...
            <?php } ?>
                ...
                <td class='text' colspan="2">
                    <input checked="checked" type="radio" name="radio_type_of_coverage" id="radio_type_of_coverage1"
                           value="self" onClick="make_self();"/>
		    <?php echo xlt('Self'); ?>
		    <?php if($patdata['provider'] != '' && $patdata['provider'] > 0){ ?>
                    <input type="radio" name="radio_type_of_coverage" id="radio_type_of_coverag2" value="insurance"
                           checked="checked"
                           onClick="make_insurance();"/>
		...
		<td>
            ...
            <!--<tr id="tr_radio1" style="display: none">
                ...
                <td class='text' colspan="2" style="display: none">
	    ...
	    // Not excussed past encounters will be set as no show and new billing entry
	    $getPastEncounters = sqlStatement("SELECT GROUP_CONCAT(b.encounter) as encounter, fe.appt_id as eid FROM billing b left join form_encounter fe on fe.encounter = b.encounter where b.pid = ? and DATE_FORMAT(fe.date, '%Y-%m-%d') < ? and b.activity != 0 group by b.encounter", [$_SESSION['pid'], date('Y-m-d')]);
	    $getAmtFine = sqlQuery("select p.pr_price, c.modifier from codes c left join prices p on p.pr_id = c.id where c.code = ? and p.pr_level = ?",['fine', 'standard']);
	    while($row = sqlFetchArray($getPastEncounters)) {
		    sqlStatement("update billing set activity = 0 where encounter = ?", [$row['encounter']]);
		    $updateAppointment = sqlQuery ("update openemr_postcalendar_events set  pc_apptstatus = ? where pc_eid = ?", ['?', $row['eid']]);
		    sqlInsert("insert into billing(date, pid, encounter, fee, units, activity, billed, code_type, code, modifier) values (now(), ?,?,?,?,?,?,'CPT4','fine',?)", [$_SESSION['pid'], $row['encounter'], text(bucks($getAmtFine['pr_price'])), 1, 1, 0, $getAmtFine['modifier']]);
	    }

	    $pastFineAmt = sqlQuery("select sum(fee) as fee from billing where code = ? and pid = ? and activity != 0", ["fine", $_SESSION['pid']]);
            $encs = array();
            // Get the unbilled service charges and payments by encounter for this patient.
            //
           /* $currDate = date('Y-m-d');
            $query = "SELECT fe.encounter, fe.reason, b.code_type, b.code, b.modifier, b.fee,oc.pc_catname as visitName, " .
                "LEFT(fe.date, 10) AS encdate ,fe.last_level_closed " . "FROM  form_encounter AS fe left join openemr_postcalendar_categories oc on oc.pc_catid=fe.pc_catid left join billing AS b  on " .
                "b.pid = ? AND b.activity = 1  AND " . "b.code_type != 'TAX' AND b.fee != 0 " . "AND fe.pid = b.pid AND fe.encounter = b.encounter " .
                "where fe.pid = ? and fe.date >= ? " . "ORDER BY fe.date limit 1";
	    $bres = sqlStatement($query, array($pid, $pid,$currDate));*/
			$query = "SELECT fe.encounter, fe.reason, b.code_type, b.code, b.modifier, b.fee, ope.pc_title, " .
				"LEFT(fe.date, 10) AS encdate ,fe.last_level_closed " . "FROM  form_encounter AS fe left join billing AS b  on " .
				"b.pid = ? AND b.activity = 1  AND " . "b.code_type != 'TAX' AND b.fee != 0 " . "AND fe.pid = b.pid AND fe.encounter = b.encounter " .
				" left join openemr_postcalendar_events ope on ope.pc_eid = fe.appt_id " .
 		"where fe.pid = ? " . "ORDER BY fe.date asc";
			$bres = sqlStatement($query, array($pid, $pid));
	    //
	    while ($brow = sqlFetchArray($bres)) {
                $key = (int)$brow['encounter'];
                if (empty($encs[$key])) {
                    $encs[$key] = array('encounter' => $brow['encounter'], 'date' => $brow['encdate'], 'last_level_closed' => $brow['last_level_closed'], 'charges' => 0, 'payments' => 0, 'reason' => $brow['pc_title']
                    );
                }
		    ...
		    //}else
			//    $encs[$key]['past_pending_fee'] += $brow['fee'];
                    // Add taxes.
                    $sql_array = array();
		...
		if($brow['encdate'] > date('Y-m-d')){
            ...
            $query = "SELECT fe.encounter, fe.reason, s.drug_id, s.fee, " .
                ...
                "where fe.pid = ? and fe.date >= ? " . "ORDER BY s.encounter limit 1";

            $dres = sqlStatement($query, array($pid, $pid,$currDate));
            //
		    ...
		    $duept = $brow['amount'] + $srow['amount'] - $drow['payments'] - $drow['adjustments'];
                ...
                echoLine("form_upay[$enc]", $dispdate, $value['charges'], $dpayment_pat, ($dpayment + $dadjustment), $duept, $reason, $inscopay, $patcopay);
            }
    ...
    <script>
	function sessionApptChange(){
	console.log("dd");
                let liburl = './lib/paylib.php?sessionExcused=true';
                $.ajax({
                        url : liburl,
                        success: function (data, textStatus, jqXHR) {
                                if(data.trim() != true)
