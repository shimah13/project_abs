...
// Change appointment status to Noshow and 0 the fee amount when patient excused
if($_GET['sessionExcused'] == 'true') {
	...
	if(!empty($selectAppointment)) {
		$updateAppointment = sqlQuery ("update openemr_postcalendar_events set  pc_apptstatus = ? where pc_eid = ?", ['excuse', $selectAppointment['pc_eid']]);
		$selectEncounter = sqlQuery("select encounter from form_encounter where appt_id =? ", [$selectAppointment['pc_eid']]);
