...
class SocialHistoryService extends BaseService
{
    ...
    private function insertRecord($record)
    {
        ...
        try {
            ...
            $processingResult->addData($insertId);
        } catch (\Exception $ex) {
            (new SystemLogger())->error($ex->getMessage(), ['trace' => $ex->getTraceAsString()]);
            $processingResult->addInternalError("Error saving record to database");
            return $processingResult;
        }
